...
void *handle_connection(void *thrd_args)
{
    ...
    args = (hdl_conn_args_t *)thrd_args;
    new_sock_conn = args->new_sock_conn;
    cli_addr      = args->cli_addr;
    cli_len       = args->cli_len;
    debg_ofp      = args->debg_ofp;
    bzero(buf, MAX_BUFFER);
    args->status = recv(new_sock_conn, buf, MAX_BUFFER, 0);
    if (args->status < 0 || strncmp(buf, "", strlen(buf))==0) {
    ...
    parse_http_request(buf, &req_info, debg_ofp);
    if (is_persistent(req_info) != 0) {
	args->status = respond_to_http(new_sock_conn, req_info, debg_ofp);
	fprintf(debg_ofp, "DEBUG: Not Persistant connection\n");
	fflush(debg_ofp);
	close(new_sock_conn);
    } else {
	args->status = wait_for_and_hdl_persistant_conn(new_sock_conn,
						  req_info, debg_ofp);
